...
This script provides an overview of the genomes available for downstream analysis.
Starting from two input datasets (Seed and Non-Seed binary matrices),

It summarizes the taxonomic distribution of a genome dataset from GTDB-formatted taxonomy strings
and generates summary statistics and visualizations:

- Barplot (unique taxa per rank, top 10 phyla, top 10 genera)
...
from config import COMPACT_METADATA_TSV, EXPLORATORY_PLOTS_DIR
...
# Plot
# ------------------------
def plot_taxonomic_summary(data, out_path):
    ...
    species_count = df_taxa.loc[df_taxa["Rank"] == "Species", "Count"].iloc[0]
    other_max = df_taxa.loc[df_taxa["Rank"] != "Species", "Count"].max()
    ...
    fig.suptitle(
        f"Number of Genomes per Taxonomic Rank in the Study Dataset (N = {total_genomes:,} Genomes)",
        y=0.98,
        fontsize=15,
    )
    ...
    plt.savefig(
        out_path / "genomes_per_taxonomic_rank.png", dpi=300, bbox_inches="tight"
    )
    plt.close(fig)
    ...
    plt.pie(
        values,
        labels=labels,
        autopct=lambda pct: f"{int(round(pct*total/100)):,} ({pct:.1f}%)",  # avoid float rounding errors
        startangle=90,
    ...
    plt.axis("equal")
    plt.tight_layout()
    plt.savefig(out_path / "genomes_per_domains.png", dpi=300, bbox_inches="tight")
    plt.close(pie)
    ...
    plt.tight_layout()
    plt.savefig(
        out_path / "top10_phyla_with_most_genomes.png", dpi=300, bbox_inches="tight"
    )
    plt.close(fig_2)
    ...
    plt.tight_layout()
    plt.savefig(
        out_path / "top10_genera_with_most_genomes.png", dpi=300, bbox_inches="tight"
    )
    plt.close(fig_3)
    return None
...
def main():
    # Load metadata
    df = load_data(COMPACT_METADATA_TSV, filetype="csv")
    ...
    plot_taxonomic_summary(df, out_path)
