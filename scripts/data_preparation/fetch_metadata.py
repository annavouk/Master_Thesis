...
- Maps KEGG IDs compounds from ModelSEED compounds from binary pickles
...
Usage:
    python3 scripts/data_preparation/fetch_metadata.py --source kegg
    python3 scripts/data_preparation/fetch_metadata.py --source patric
    python3 scripts/data_preparation/fetch_metadata.py --source prego
    python3 scripts/data_preparation/fetch_metadata.py --source kegg --out_json metadata/kegg.json --out_failed metadata/kegg_failed.txt
"""
...
from config import (
    METADATA_DIR,
    SEEDS_PICKLE,  # input PATRIC + to extract compounds
    NON_SEEDS_PICKLE,  # input to extract compounds
    COMPOUNDS_TSV,  # input to extract compounds and map ModelSEED -> KEGG IDs
    PREGO_INPUT_CSV,  # input PREGO (merged PATRIC + GTDB metadata with column ncbi_taxid)
    PATRIC_METADATA_JSON,  # output PATRIC
    FAILED_PATRIC_IDS_TXT,  # output PATRIC
    KEGG_DATA_JSON,  # output KEGG
    FAILED_KEGG_IDS_TXT,  # output KEGG
    PREGO_ENVIRONMENTS_JSON,  # output PREGO
    FAILED_PREGO_IDS_TXT,  # output PREGO
)
...
def load_patric_ids():
    ...
    df = load_data(SEEDS_PICKLE, filetype="pickle")
    return sorted(list(df.index))


def load_taxids():
    """Extract unique, cleaned NCBI TaxIDs from merged metadata."""
    data = load_data(PREGO_INPUT_CSV, filetype="csv")
    ...
    ids = (
        ...
        .astype(str)
        .str.strip()
        .str.extract(r"^(\d+)$", expand=False)  # κρατά μόνο καθαρά ψηφία
        .dropna()
        .drop_duplicates()
        .tolist()
    )
    ...
    print(f"[prego] Found {len(ids)} unique ncbi_taxid from PATRIC metadata")
    return ids


def cpds_preparation():
    """Extract ModelSEED compounds and map them to KEGG IDs"""
    seeds = load_data(SEEDS_PICKLE, filetype="pickle")
    non_seeds = load_data(NON_SEEDS_PICKLE, filetype="pickle")
    seed_set, non_seed_set = get_compound_sets(seeds, non_seeds)
    modelseed_union = seed_set | non_seed_set

    comp = load_data(COMPOUNDS_TSV, filetype="tsv")
    sub = comp[comp["id"].isin(modelseed_union)][["id", "aliases"]].copy()

    # Find the first KEGG compound (Cxxxxx) per row
    sub["KEGG_ID"] = extract_kegg_ids(sub, column="aliases")

    # Unique Cxxxxx
    ids = sub["KEGG_ID"].dropna().astype(str).str.strip().drop_duplicates().tolist()

    print(
        f"[kegg] mapped {sub['KEGG_ID'].notna().sum()} / {len(sub)} ModelSEED → KEGG; unique KEGG IDs: {len(ids)}"
    )
    if not ids:
        print("[warn] No KEGG Cxxxxx derived from pickles+compounds.tsv")
    return ids
