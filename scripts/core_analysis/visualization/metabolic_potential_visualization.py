...
Visualize the data using histograms, boxplots, scatterplots, violin plots and barplots
...
def main():
    ...
    # Plot boxplots for seeds/non-seeds
    plot_boxplot(
        df,
        column="Ratio",
        ...
        save_plot=True,
        plot_path=METABOLIC_POTENTIAL_PLOTS_DIR / "boxplot_ratio.png",
        method="2std",
    )
    ...
    pearson_r, p1, spearman_r, p2 = correlation_stats(
        df, x="genome_size_Mbp", y="Ratio", log_x=False, log_y=False
    )
    with open(stats_file, "a", encoding="utf-8") as f:
