...
from config import( 
    SEEDS_PICKLE, 
    NON_SEEDS_PICKLE,
    COMPACT_METADATA_TSV,
    AMINOACIDS_TSV,
    OUTPUT_DIR,
    AMINOACIDS_PLOTS_DIR,
)
...
def analyze_auxotrophy(seed_matrix, nonseed_matrix, aa_df, metadata_df):
    ...
    aa_cpd_ids = aa_df["SEED_ID"].astype(str).tolist()
    ...
    nonseed_aa_matrix = ensure_binary(nonseed_matrix.reindex(columns=common_aa, fill_value=0))
    ...
    summary_df = pd.DataFrame({
        'Auxotrophy': auxotrophy_per_genome,
        'Prototrophy': prototrophy_per_genome
    })
    ...
    labels = aa_df.set_index('SEED_ID')['compound_name'].astype(str)
    ...
    # Merge with taxonomy
    merged = (
        both.reset_index(names="PATRIC")
        .merge(
            metadata_df[["patric_id", "gtdb_taxonomy"]],
            left_on="PATRIC", right_on="patric_id", how="left"
            )
    )
    ...
    return summary_df, counts_df, merged, list(seed_pref.columns), list(nonseed_pref.columns)


# ------------------------
# Taxonomic aggregation
# ------------------------
def aggregate_by_taxon(merged, seed_cols_pref, nonseed_cols_pref, ranks=("phylum", "family", "genus")):
    ...
    for rank in ranks:
        ...
        aux_counts = rank_group[seed_cols_pref].sum(min_count=1)    # number of seed AAs per rank
        proto_counts = rank_group[nonseed_cols_pref].sum(min_count=1)    

        rank_counts = merged.groupby(rank, dropna=False).size()    # Number of total genomes per rank
...
def plot_stacked(
    ...
    labels = aa_df.assign(SEED_ID=aa_df["SEED_ID"].astype(str)).set_index("SEED_ID")["compound_name"]
    ...
    title_mode = "Composition (100% per AA)" if mode == "composition" else "Percent within taxon"
...
def main():
    ...
    # Analyze auxotrophy
    summary_df, counts_df, merged, seed_cols_pref, nonseed_cols_pref = analyze_auxotrophy(seeds, non_seeds, aa_df, metadata_df)
    ...
    for rank, res in results.items():
        # counts + percentages
        aux_df = pd.concat([res["aux"]["counts"], res["aux"]["percentages"]], axis=1, keys=["counts", "percentages"])
        proto_df = pd.concat([res["proto"]["counts"], res["proto"]["percentages"]], axis=1, keys=["counts", "percentages"])

        combined = pd.concat([aux_df, proto_df], axis=1, keys=["Auxotrophy", "Prototrophy"])
    ...
    plot_summary_hist(summary_df, n_amino_acids=n_aa, log_scale=True, save_path=plot_dir/"aa_summary_hist.png")
