...
from config import( SEEDS_PICKLE, # input
                   NON_SEEDS_PICKLE,
                   COMPOUND_SUMMARY_TSV,
                   SEED_NODE_FREQUENCY_PLOTS_DIR,
                   OUTPUT_DIR
                   )
...
def compute_coverage(seeds_df, non_seeds_df):
    ...
    df_cov = pd.DataFrame({
        "percent_as_seed": seed_freq,
        "percent_as_non_seed": non_seed_freq
    })
...
# Visualizations
# ------------------------
def plot_seed_histogram(seed_freq_df, bins=30, log_scale=True, color="slateblue", save_path=None):
    """Histogram: Distribution of seed frequencies across compounds."""
...
def plot_top_bar(df, column, top_n=20, color="teal", title=None, save_path=None):
    ...
    if save_path:
        plt.savefig(save_path/f"top_{top_n}_seed_nodes.png", dpi=300, bbox_inches="tight")
...
def plot_coverage_histogram(df_cov, bins=40, logy=False, save_path=None):
    ...
    # Histogram seeds
    ax.hist(
        df_cov["percent_as_seed"], bins=bins, 
        alpha=0.6, label="Seeds", 
        color="steelblue", edgecolor="black"
    )
    # Histogram non-seeds
    ax.hist(
        df_cov["percent_as_non_seed"], bins=bins, 
        alpha=0.5, label="Non-Seeds", 
    ...
    ax.axvline(mean_seed, color="red", linestyle="--", label=f"Mean seed = {mean_seed:.1f}%")
    ax.axvline(mean_nonseed, color="blue", linestyle="--", label=f"Mean non-seed = {mean_nonseed:.1f}%")
    ...
    # Save
    if save_path:
        fname = "seed_non_seed_genome_coverage_log.png" if logy else "seed_non_seed_genome_coverage.png"
...
def main():
    ...
    seed_freq_df = seed_freq_df.merge(compounds_map[["ModelSEED_ID", "compound_name"]], on="ModelSEED_ID", how="left")
    ...
    df_cov.to_csv(Path(OUTPUT_DIR) / "genome_coverage_per_compound.tsv", sep="\t", index=False)
