...
- Bar plot of top KEGG pathways in which participate the compounds that are only seeds
...
def plot_venn(seed_compounds, nonseed_compounds):
...
def plot_kegg_modules_per_compound(cpd_df):
    ...
    ax = cpd_df["KEGG_module_count"].hist(
...
def plot_top_pathways(top_pathways, subset_name, cpd_df, compounds_set):
    ...
    subset_pathways = (
        subset_df["KEGG_pathways"].dropna().str.split(",").explode().str.strip()
    )
    ...
    plt.title(
        f"Top KEGG Pathways among {subset_name} Compounds(N={len(compounds_set)})"
    )
    plt.tight_layout()
    plt.show()
...
def main():
    ...
    output_path = Path(OUTPUT_DIR) / "unique_kegg_reactions.tsv"
    
    # Unique KEGG entities
    count_unique(cpd_df, "KEGG_modules")
    count_unique(cpd_df, "KEGG_pathways")
    reactions = count_unique(cpd_df, "KEGG_reactions", return_list=True)
    unique_reactions = pd.DataFrame({"KEGG_reaction_ID": sorted(reactions)})
    unique_reactions.to_csv(output_path, sep="\t", index=False)
    print(f"Saved {len(unique_reactions)} unique reactions to {output_path}")

    # Venn
    seed_compounds, nonseed_compounds = get_compound_sets(seed_df, nonseed_df)
    only_seed, only_nonseed, common = plot_venn(seed_compounds, nonseed_compounds)
    ...
    # Top pathways
    for subset_name, subset_set in zip(
        ["Seed Only", "Non-Seed Only", "Seed ∩ Non-Seed"],
