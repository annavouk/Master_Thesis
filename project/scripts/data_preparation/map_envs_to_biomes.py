...
RULES_REGEX = [
    (
        r"soil|rhizosphere|forest soil|bulk soil|paddy field|forest|grassland|woodland|farm|field|vegetated area|agricultural feature|peatland|cropland biome",
        "Soil",
    ),
    (
        r"sea|marine|ocean|brackish|lagoon|coastal|coast|harbor|beach|estuarine biome|mangrove biome",
        "Marine",
    ),
    (
        r"lake|pond|wetland|fresh\s*water|ground\s*water|river|stream|spring|aquifer|watercourse",
        "Freshwater",
    ),
    (
        r"sludge|waste|sewage|bioreactor|landfill|compost|treatment plant|gold mine|animal manure|building|mine|petroleum|oil|urban|food processing factory|dairy|cultivated environment|village biome|anthropogenic environment",
        "Engineered",
    ),
    (
        r"glacier|permafrost|hot\s*spring|hydrothermal vent|saline|acid|tundra|extreme|desert|alkaline|high temperature|ice mass|glacial feature|fumarole",
        "Extreme",
    ),
    (r"gut|intestin|oral|skin|host|hospital", "Host"),
]
...
# Functions
# -------------------------------
def preprocess_envs(df: pd.DataFrame):
...
def main():
    ...
    # Save to file
    out_path = Path(COMPACT_METADATA).with_name("compact_metadata_with_biome.tsv")
