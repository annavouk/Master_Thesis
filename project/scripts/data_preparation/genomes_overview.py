...
def plot_taxonomic_summary(data):
    ...
    sns.barplot(data=df_taxa, x="Rank", y="Count", hue="Rank", palette="Blues_d", legend=False, ax=ax1)
    sns.barplot(data=df_taxa, x="Rank", y="Count", hue="Rank", palette="Blues_d", legend=False, ax=ax2)
    ...
    for bar, row in zip(bars1, df_taxa.itertuples()):
        ...
        if value > break_threshold:
            ax1.text(x, y + offset, label, ha='center', va='bottom',
    ...
    for bar, row in zip(bars2, df_taxa.itertuples()):
        ...
        y = bar.get_height()
        if value <= break_threshold:
            ax2.text(x, y + offset, label, ha='center', va='bottom',
                     fontsize=12, fontweight='bold')
   
    fig.suptitle(f"Number of Unique Taxonomic Groups per Rank in the Study Dataset (N = {total_genomes:,} Genomes)", pad=10, fontsize=15)
    ...
    plt.pie(domain_counts, labels=domain_counts.index,
            ...
            startangle=90, colors=sns.color_palette("pastel"), textprops={'fontsize': 12})
    plt.title(f"Distribution of Genomes Across Domains in the Study Dataset\n(N = {total_genomes:,} Genomes)", pad=20)
    plt.axis("equal")
    plt.tight_layout()
    plt.show()
    ...
    ax = sns.barplot(data=top_phyla, x="Count", y="Phylum", hue="Phylum", palette="viridis", legend=False)
    plt.xlabel("Number of Genomes")
    plt.ylabel("Phylum")
    plt.title(f"Top 10 Most Abundant Phyla in the Study Dataset\n"
    f"({top_phyla_total:,} of {total_genomes:,} Genomes)")

    for i, row in top_phyla.iterrows():
        ax.text(row["Count"] + max(top_phyla["Count"])*0.01, i, f'{row["Count"]:,}', va='center', fontsize=12)
    ...
    plt.figure(figsize=(10, 6))
    ax = sns.barplot(data=top_genera, x="Count", y="Genus", hue="Genus", palette="magma", legend=False)
    plt.xlabel("Number of Genomes")
    plt.ylabel("Genus")
    plt.title(f"Top 10 Most Abundant Genera in the Study Dataset\n"
    f"({top_genera_total:,} of {total_genomes:,} Genomes)")

    for i, row in top_genera.iterrows():
        ax.text(row["Count"] + max(top_genera["Count"])*0.01, i, f'{row["Count"]:,}', va='center', fontsize=12)
