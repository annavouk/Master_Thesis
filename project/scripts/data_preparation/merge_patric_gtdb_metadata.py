...
- Outputs merged table with taxonomic + genome information
"""
...
from config import PATRIC_METADATA_JSON, PREGO_ENVIRONMENTS_JSON, GTDB_METADATA, COMPACT_METADATA
...
def clean_patric_accessions(df):
    ...
    df_filtered["root_assembly"] = (
        df_filtered["assembly_accession"].str.split(".").str[0]
    )
    return df_filtered
...
def clean_gtdb_metadata(df):
    ...
    # From NCBI
    df_filtered["assembly_accession_ncbi"] = (
        df_filtered["ncbi_genbank_assembly_accession"]
        .astype(str)
        .str.replace(r"^(GCA_|GCF_)", "", regex=True)
    )
    df_filtered["root_assembly_ncbi"] = (
        df_filtered["assembly_accession_ncbi"].str.split(".").str[0]
    )
    # From GTDB 'accession'
    df_filtered["assembly_accession_acc"] = (
        df_filtered["accession"]
        .astype(str)
        .str.replace(r"^(RS_GCF_|GB_GCA_|GB_GCF_|GCA_|GCF_)", "", regex=True)
    )
    df_filtered["root_assembly_acc"] = (
        df_filtered["assembly_accession_acc"].str.split(".").str[0]
    )
    return df_filtered
...
# PREGO enrichment
# ------------------------
def add_prego_envs(df, prego_json_path):
    """Add lit_envs, meta_envs, all_envs columns from PREGO to metadata table."""
    with open(prego_json_path) as f:
        prego = json.load(f)

    def get_envs(taxid, key):
        return "; ".join(prego.get(str(taxid), {}).get(key, []))

    df["lit_envs"] = df["ncbi_taxid"].astype(str).map(lambda tx: get_envs(tx, "lit_envs"))
    df["meta_envs"] = df["ncbi_taxid"].astype(str).map(lambda tx: get_envs(tx, "meta_envs"))

    def union_envs(row):
        lit_set = {e for e in row["lit_envs"].split("; ") if e} if row["lit_envs"] else set()
        meta_set = {e for e in row["meta_envs"].split("; ") if e} if row["meta_envs"] else set()
        return "; ".join(sorted(lit_set | meta_set))

    df["all_envs"] = df.apply(union_envs, axis=1)
...
def dual_merge(patric_df, gtdb_df, output_path=None):
    """Merge first on root_assembly vs root_assembly_ncbi, then unmatched on root_assembly vs root_assembly_acc. Returns single, deduplicated merged DataFrame."""
    # First merge: PATRIC vs GTDB by NCBI accession
    merged_1 = pd.merge(
        patric_df,
        gtdb_df,
        left_on="root_assembly",
        right_on="root_assembly_ncbi",
        how="left",
    ...
    unmatched_patric = unmatched[
        ["patric_id", "assembly_accession", "genome_length", "root_assembly"]
    ]
    merged_2 = pd.merge(
        unmatched_patric,
        gtdb_df,
        left_on="root_assembly",
        right_on="root_assembly_acc",
        how="left",
    )
    print(
...
def main():
    ...
    merged = add_prego_envs(merged, PREGO_ENVIRONMENTS_JSON)

    # Save final
    merged.to_csv(COMPACT_METADATA, index=False)
    print(f"Final merged metadata + environments saved to: {COMPACT_METADATA}")
